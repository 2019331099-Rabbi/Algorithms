#include<windows.h>
#ifdef __APPLE__
#include <GLUT/glut.h>
#else
#include <GL/glut.h>
#endif

#include <stdlib.h>
#include <bits/stdc++.h>
using namespace std;

// Orth
int xmin = -400, xmax = 400;
int ymin = -400, ymax = 400;


// Square
int wxmin = -100, wymin = -100;
int wxmax = 100, wymax = 100;

struct Point {
    int x;
    int y;
    Point(int _x, int _y) {
        x = _x;
        y = _y;
    }
};


long double slope(Point p1, Point p2);
void setPixel(Point p);
void drawAxis();
void drawWindow();

void findPoints(Point p, vector <Point> &points, int radius)
{
    int x = p.x, y = p.y;
    points.push_back(Point(x - radius, y - radius));
    points.push_back(Point(y - radius, x - radius));

    points.push_back(Point(-y + radius, -x + radius));
    points.push_back(Point(-x + radius, -y + radius));

    points.push_back(Point(x - radius, -y + radius));
    points.push_back(Point(-y + radius, x - radius));

    points.push_back(Point(-x + radius, y - radius));
    points.push_back(Point(y - radius, -x + radius));
}

void bresenhamCircleDrawing(Point center, int radius)
{
    int x = 0, y = radius;
    int d = 3 - 2 * radius;

    vector <Point> points;
    while (x <= y) {
        findPoints(Point(x, y), points, radius);
        if (d < 0) d = d + 4*x + 6;
        else {
            d = d + 4*(x-y) + 10;
            y--;
        }
        x++;
    }

    // Translate back
    for (auto &p: points) {
        p.x = p.x + center.x;
        p.y = p.y + center.y;
    }

    for (auto point: points) setPixel(point);
}

void display(void)
{
    glClear(GL_COLOR_BUFFER_BIT);
    drawAxis();
    drawWindow();
    glColor3f(1.0, 1.0, 1.0);  // White

    glBegin(GL_POINTS);

    Point center = Point((wxmin + wxmax) / 2, (wymin + wymax) / 2);
    int radius = (wxmax - wxmin) / 2;
    bresenhamCircleDrawing(center, radius);

    glEnd();
    glFlush();
}

void init(){
    glClearColor(0.0,0.0,0.0,0.0);
    glMatrixMode(GL_PROJECTION);
    glLoadIdentity();
    glOrtho(xmin, xmax, ymin, ymax, -10, 10);
}


int main(int argc, char *argv[])
{
    glutInit(&argc, argv);
    glutInitWindowSize(640, 640);
    glutInitWindowPosition(10,10);
    glutInitDisplayMode(GLUT_RGB | GLUT_SINGLE);

    glutCreateWindow("Demo");
    init();
    glutDisplayFunc(display);
    glutMainLoop();

    return EXIT_SUCCESS;
}


long double slope(Point p1, Point p2)
{
    return (long double)(p2.y - p1.y) / (long double)(p2.x - p1.x);
}

void setPixel(Point p)
{

    glVertex3f(p.x, p.y, 0.0);
}

void drawWindow()
{
    glColor3f(0.0, 1.0, 0.0);  // White color for axes
    glBegin(GL_LINES);
    glVertex2i(wxmin, wymin);
    glVertex2i(wxmax, wymin);

    glVertex2i(wxmax, wymin);
    glVertex2i(wxmax, wymax);

    glVertex2i(wxmax, wymax);
    glVertex2i(wxmin, wymax);

    glVertex2i(wxmin, wymax);
    glVertex2i(wxmin, wymin);
    glEnd();
}

void drawAxis()
{
    glColor3f(0.0, 1.0, 0.0);  // White color for axes
    glBegin(GL_LINES);

    // X-axis
    glVertex2f(xmin, 0);
    glVertex2f(xmax, 0);

    // Y-axis
    glVertex2f(0, ymin);
    glVertex2f(0, ymax);

    glEnd();
}
